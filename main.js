...
var GeminiNoteSettingTab = class extends import_obsidian.PluginSettingTab {
  ...
  display() {
    ...
    const addBlockSetting = (name, desc) => {
      ...
      infoEl.style.margin = "0 0 10px 0";
    ...
    new import_obsidian.Setting(containerEl).setName("Model Name").setDesc("The Gemini model to use (e.g., gemini-1.5-pro-latest)").addText((text) => text.setPlaceholder("gemini-1.5-pro-latest").setValue(this.plugin.settings.modelName).onChange(async (value) => {
      this.plugin.settings.modelName = value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian.Setting(containerEl).setName("Task Instructions Folder").setDesc("Path to folder containing instruction templates").addText((text) => text.setPlaceholder("Templates/Instructions").setValue(this.plugin.settings.instructionsFolder).onChange(async (value) => {
      this.plugin.settings.instructionsFolder = value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian.Setting(containerEl).setName("Default Context").setDesc("Default context selection in the modal").addDropdown((dropdown) => dropdown.addOption("selection_only", "Selection Only").addOption("selection_and_full_note", "Selection + Full Parent Note").setValue(this.plugin.settings.defaultContext).onChange(async (value) => {
      this.plugin.settings.defaultContext = value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian.Setting(containerEl).setName("Default Output Action").setDesc("Default action to take with the generated content").addDropdown((dropdown) => dropdown.addOption("create_note", "Create New Note").addOption("replace_selection", "Replace Selected Text").addOption("insert_after", "Insert at Selection").setValue(this.plugin.settings.defaultOutputAction).onChange(async (value) => {
      this.plugin.settings.defaultOutputAction = value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian.Setting(containerEl).setName("Default Save Location").setDesc("Default subfolder for generated notes (only applied when creating a new note)").addText((text) => text.setValue(this.plugin.settings.defaultSaveLocation).onChange(async (value) => {
...
var GenerationConfigModal = class extends import_obsidian2.Modal {
  ...
  async onOpen() {
    ...
    new import_obsidian2.Setting(contentEl).setName("Output Action").setDesc("How to handle the generated result").addDropdown((dropdown) => {
      ...
      dropdown.onChange((value) => {
        this.selectedOutputAction = value;
        this.updateSaveLocationVisibility(saveLocationSettingEl);
      });
    });
    const saveLocationSetting = new import_obsidian2.Setting(contentEl).setName("Save Location (Subfolder)").setDesc("Leave empty to save in the same folder as parent").addText((text) => {
    ...
    new import_obsidian2.Setting(contentEl).addButton(
      (btn) => btn.setButtonText("Generate").setCta().onClick(() => {
        this.submitForm();
      })
    );
  ...
  updateSaveLocationVisibility(element) {
  ...
  submitForm() {
    if (!this.selectedInstructionPath) {
      new import_obsidian2.Notice("Please select a task instruction.");
      return;
    }
    this.close();
    this.onSubmit({
  ...
  getInstructionFiles() {
    ...
    if (!folder)
      return [];
    if (folder.children) {
      return folder.children.filter((f) => f instanceof import_obsidian2.TFile && f.extension === "md");
    }
    return [];
  }
  onClose() {
...
${JSON.stringify(payload, null, 2)}`;
  ...
  async generateWithCustomHost(prompt) {
    var _a, _b, _c, _d, _e;
